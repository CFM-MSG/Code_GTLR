...
class Framework:
    ...
    def step_2_environment(self, *args, **kwargs):
        ...
        os.makedirs(os.path.join(self.output, "best"), exist_ok=True)
        os.makedirs(os.path.join(self.output, "ckpt"), exist_ok=True)
    ...
    def save_checkpoint(self, filename="checkpoint.pth", **kwargs):
        ...
        path = os.path.join(self.output, "ckpt", filename)
    ...
    def save_model(self, name="model.pth"):
        if not ext.distributed.is_main_process():
            return
        path = os.path.join(self.output, "best", name)
